...
export class TankCheckLiquid_BottomUp extends TankCheckPro {
  public constructor(obj: any, loaded: boolean, hwFamily: HardwareFamily, hwVersionNumber: number) {
    ...
    this.category = SensorCategory.BottomWater
    this.isWater = true
    if (this.name == get_i18n('new_h20_device') || this.name == get_i18n('new_device')) {
      this.name = get_i18n('new_pro_h20_device')
    }
  }

  public convertLevelToInches(tof: number): number {
    // Spend of sound in water
    let c: number = 1481
